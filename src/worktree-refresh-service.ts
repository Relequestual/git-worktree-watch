...
export class WorktreeRefreshService {
    ...
    this.disposables.push(
      this.gitApi.onDidOpenRepository((repository) => {
        this.registerRepositoryWatcher(repository).then(
          ...
          (error) =>
            this.log(
              `Failed to register watcher for ${repository.rootUri.fsPath}: ${formatError(error)}`
            )
        );
      }),
      vscode.workspace.onDidChangeConfiguration((event) => {
        ...
        this.refreshWatchers().then(
          ...
          (error) =>
            this.log(`Failed to refresh watchers after configuration change: ${formatError(error)}`)
        );
      })
    );
  ...
  private async registerRepositoryWatcher(repository: GitRepository): Promise<void> {
    ...
    const worktreesDirectoryWatcher = vscode.workspace.createFileSystemWatcher(
      new vscode.RelativePattern(commonGitDir, 'worktrees')
    );
    const worktreeMetadataWatcher = vscode.workspace.createFileSystemWatcher(
      new vscode.RelativePattern(vscode.Uri.file(worktreesPath), '*')
    );
    ...
    for (const watcher of watchers) {
      watcher.onDidCreate(
        (uri) => this.scheduleRescan(`worktree metadata created: ${uri.fsPath}`),
        undefined,
        disposables
      );
      watcher.onDidChange(
        (uri) => this.scheduleRescan(`worktree metadata changed: ${uri.fsPath}`),
        undefined,
        disposables
      );
      watcher.onDidDelete(
        (uri) => this.scheduleRescan(`worktree metadata deleted: ${uri.fsPath}`),
        undefined,
        disposables
      );
    }
  ...
  private scheduleRescan(reason: string): void {
    if (!getAutoRefresh()) {
      this.log(
        `Skipped scheduled rescan because automatic refresh is disabled. Reason: ${reason}.`
      );
      return;
    }
  ...
  private async rescanRepository(repository: GitRepository): Promise<void> {
    ...
    try {
      ...
      const openRepositoryRoots = new Set(
        this.gitApi.repositories.map((repo) => normalizeFsPath(repo.rootUri.fsPath))
      );
